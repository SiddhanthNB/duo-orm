...
def _generate_files(migrations_dir: Path, db_object_path: str):
    ...
    env_py_content = env_template.format(db_object_import_path=db_object_path)
...
def get_alembic_config() -> Config:
    ...
    """
    user_config = _get_config()
    migrations_dir = user_config["migrations_dir"]
    db_object_path = user_config["db_object_path"]

    # This is the path to the INI file inside the user's project
    config_file_path = str(Path.cwd() / migrations_dir / "alembic.ini")
